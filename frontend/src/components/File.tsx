...
const File = ({ unattachFile, file }: FileProps) => {
  ...
  const { isSuccess, progress, uploadForm } = useUploadForm<UploadingResponse>(
    "http://212.192.35.17/upload"
  );
  ...
  const upload = async () => {
    ...
    const { pdf, qr, paymentInfo, content } = await uploadForm(formData);
  ...
  const downloadQR = () => downloadImage(qr?.png!, "png");
  ...
  return (
    <div className="w-full flex  gap-2 items-center mt-3 max-w-xs">
      {isDoc && <BsFiletypeDoc className="w-24 h-24 text-neutral" />}
      {isDocx && <BsFiletypeDocx className="w-24 h-24 text-neutral" />}
      {isPdf && <BsFileEarmarkPdf className="w-24 h-24 text-neutral" />}
      <div className="w-full">
        <div className="flex justify-between items-center mb-1">
          <p className=" text-neutral  font-semibold ">
            {fileName.length >= 20 ? fileName.substring(20) + "..." : fileName}
          </p>
